...
計測可能な反応があり、有料本の決済導線につながる）、**このブログ = 名刺・ポートフォリオ**（CMP/Wasm 製で
あること自体が実績で、動く Compose デモという Zenn に無い武器を持つ）。書き始める前に、そのネタの目的から
出し先を判定する:

- **このブログ向き**: 体験ベースの語り・ポートフォリオ性の強い話（作ったもの・愛車ネタ等）、
  このブログ自体の話（構成変更・不具合対応記）、インタラクティブデモが活きるネタ
- **Zenn 向き**: 特定の個人体験に依存しない汎用的な技術解説（広い読者への解説記事）、
  マネタイズの反応検証や有料本（Zenn 本等）への導線が目的の記事
- **両方に価値**: Zenn に本編、このブログに別切り口（デモ付き）の書き分けを提案する。
  Zenn は canonical URL 非対応のため、完全な二重投稿はしない

Zenn 向きと判定したら、理由を添えてユーザーに提案して終了する（Zenn 記事の執筆はこのスキルの範囲外。
Zenn リポジトリはこのマシンには無い）。迷うケースは一問だけ聞く:
「この記事の目的は**検証・販売**ですか、それとも**資産・名刺**ですか？」。
